...
use wasm_pkg_common::{config::Config, Registry};
use wasm_pkg_loader::{config::oci::OciRegistryConfig, oci_client, Client};
...
async fn fetch_smoke_test() {
    ...
    client_config.set_default_registry(Some(registry.clone()));
    ...
    reg_config.set_backend_type(Some("oci".to_string()));
